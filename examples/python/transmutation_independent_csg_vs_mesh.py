"""Compare independent transmutation on CSG vs CAD-mesh geometry.

Independent transmutation runs transport only ONCE to extract per-source-particle
reaction rates, then scales them by each step's source_rate.  This is much
faster than coupled mode for low-burnup / fusion scenarios.
...
Usage:
    python examples/python/transmutation_independent_csg_vs_mesh.py
"""
...
def build_mesh():
    ...
    tmpdir = tempfile.mkdtemp(prefix="yamc_dep_ind_mesh_")
...
# ---------------------------------------------------------------------------
# Run independent transmutation on a geometry
# ---------------------------------------------------------------------------
def run_transmutation(geometry, label):
    ...
    schedule = yamc.PulseSchedule([
        yamc.Pulse(rate=1e20, duration=(1, "d"), source=source),
        yamc.Pulse(rate=1e20, duration=(1, "d"), source=source),
    ...
    print(f"\n{label} independent transmutation ({elapsed:.2f}s):")
    print("  Transport ran ONCE, rates scaled per step.")
...
print("=" * 60)
print("CSG vs Mesh INDEPENDENT transmutation comparison")
