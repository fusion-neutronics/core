"""
Photon flux spectra for a photon source.

Loops over all elements with available photon data (and matching neutron data)
in the nuclear data directory, running a 1 MeV photon source simulation in a
sphere of each element.
"""
import glob
import os
import re
...
for element, isotope, neutron_path, photon_path in tqdm.tqdm(elements):
    ...
    # Source: 1 MeV isotropic photons at origin
    source = yamc.PhotonSource(
        position=(0, 0, 0),
        energy=1e6)

    # Tallies: energy-resolved photon flux
    tally = yamc.Tally(
    ...
    ax.set_title(f'Photon source: 1 MeV photons in {element} ({isotope})')
    ax.grid(True, alpha=0.3)
    ax.legend()

    plt.tight_layout()
    plt.savefig(f'flux_photon_{element}.png', dpi=150)
    plt.close()
