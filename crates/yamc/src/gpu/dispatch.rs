...
pub enum GpuDispatchError {
    ...
    /// error here (`PhotonBankOverflow`, `MaxLostParticlesExceeded`): a rank
    /// that truncates returns before the collective tally reduction while the
    /// others wait in it. Making launch-loop errors collective is a separate
    /// piece of work; until then a binding cap under MPI shows up as a stuck
    /// run rather than a message, which is still not a wrong answer.
    HistoriesTruncated {
        truncated: usize,
        launched: usize,
        max_steps: u32,
...
fn run_neutron_per_history(
    ...
    while let Some((launch_idx, launch_n)) = sched.next() {
        ...
        if has_mesh || escalated {
            ...
            let (drain_spilled, drain_relaunched) = drain_banked_neutrons(
                ctx,
                model,
                inputs,
                pack,
            ...
            spilled_total += drain_spilled;
            relaunched_total += drain_relaunched;
    ...
    Ok(GpuRunResult {
        ...
        n_bank_relaunched: relaunched_total,
    })
}
...
) -> Result<(u64, u64), GpuDispatchError> {
...
///
/// Returns `(spilled, relaunched, bank)`: how many secondaries spilled across
/// the primary launch and every generation, how many neutrons the generations
/// re-launched, and the combined bank.
#[cfg(not(target_os = "macos"))]
#[allow(clippy::too_many_arguments)]
fn drain_banked_neutrons_coupled(
    ...
    lost: &mut LostTracker,
    per_source_total: &mut [f64],
) -> Result<(u64, u64, BankedNeutrons), GpuDispatchError> {
...
pub(super) fn run_on_gpu_photon(
    ...
    Ok(GpuRunResult {
        ...
        lost_count: lost.count,
        lost: lost.records,
        // A photon-only pass transports no neutrons, so nothing can spill or
        // be re-launched.
...
fn run_on_gpu_coupled(
    ...
    while let Some((chunk_idx, chunk_sources)) = sched.next() {
        ...
        relaunched_total += chunk_relaunched;
        ...
        while drained < bank_count {
            ...
            // A banked photon transported under the same cap can truncate too.
            fail_if_truncated(&pres.alive, max_steps)?;
...
mod neutron_records_tests {
    ...
    fn record(
        ...
        f.extend((0..BANK_F64_STRIDE).map(|k| tag + k as f64 * 0.001));
        u.extend_from_slice(&[ptype, 7, 1000 + tag as u32, 2][..BANK_U32_STRIDE]);
        s.push(src);
    }

    #[test]
    fn keeps_only_neutron_records_in_order_with_their_sources() {
        let (mut f, mut u, mut s) = (Vec::new(), Vec::new(), Vec::new());
        record(&mut f, &mut u, &mut s, 1.0, PTYPE_PHOTON, 10);
        record(&mut f, &mut u, &mut s, 2.0, PTYPE_NEUTRON, 11);
        record(&mut f, &mut u, &mut s, 3.0, PTYPE_PHOTON, 12);
        record(&mut f, &mut u, &mut s, 4.0, PTYPE_NEUTRON, 13);
        // A fifth record sits past `count`, as the slots past the live count
        // of a device bank do, and must be ignored.
        record(&mut f, &mut u, &mut s, 5.0, PTYPE_NEUTRON, 14);
        let bank = BankedNeutrons {
            f64s: f,
            u32s: u,
            src: s,
            count: 4,
        ...
        assert_eq!(n.count, 2);
        assert_eq!(n.src, vec![11, 13]);
        assert_eq!(n.f64s.len(), 2 * BANK_F64_STRIDE);
        assert_eq!(n.u32s.len(), 2 * BANK_U32_STRIDE);
        assert_eq!(n.f64s[0], 2.0);
        assert_eq!(n.f64s[BANK_F64_STRIDE], 4.0);
        assert_eq!(n.u32s[2], 1002);
        assert_eq!(n.u32s[BANK_U32_STRIDE + 2], 1004);
        assert!(n
            .u32s
            .iter()
            .step_by(BANK_U32_STRIDE)
            .all(|&p| p == PTYPE_NEUTRON));
    }

    #[test]
    fn a_count_past_the_buffers_is_clamped() {
        let (mut f, mut u, mut s) = (Vec::new(), Vec::new(), Vec::new());
        record(&mut f, &mut u, &mut s, 1.0, PTYPE_NEUTRON, 3);
        let bank = BankedNeutrons {
            f64s: f,
            u32s: u,
            src: s,
