...
fn run_gpu_retry(model: &mut Model, settings: &TransportSettings) -> Result<u64, String> {
    run_gpu_retry_counts(model, settings).map(|(spilled, _relaunched)| spilled)
}

/// `(n_spilled_secondaries, n_bank_relaunched)` from a GPU run, with the same
/// transient-error retry.
fn run_gpu_retry_counts(
    model: &mut Model,
    ...
    for attempt in 0..5 {
        match yamc::gpu::run_on_gpu(model, settings) {
            Ok(r) => return Ok((r.n_spilled_secondaries, r.n_bank_relaunched)),
            Err(e) => {
                last = e.to_string();
...
///
/// Two things make this discriminating despite the spill being ~1 in 4000
/// histories. First, the dispatch reports how many neutrons the drain actually
/// re-launched, and on this non-fissile, unit-weight fixture that must equal
/// the spill count exactly: a drain that only counted spills, or dropped some,
/// fails that equality whatever the tallies say. Second, the whole-population
/// mean and std_dev comparisons hold the relaunch to the same agreement as
/// the neutron-only path, which catches a relaunch that scored wrongly
/// (double-counting or mis-attributed samples move the std_dev of every
/// history, not just the spilled ones). The photon flux from beryllium is
/// small and its statistics wide at this history count, so it is held to a
/// z-score against the two backends' own standard deviations; it is a sanity
/// check on the photon sub-pass, not a sensitive one.
#[test]
fn spilled_secondaries_drain_on_the_coupled_pass() {
    ...
    let (spilled, relaunched) =
        run_gpu_retry_counts(&mut gpu_model, &gpu_settings).expect("GPU coupled run");
    ...
    assert_eq!(
        relaunched, spilled,
        "the coupled drain re-launched {relaunched} neutrons for {spilled} spilled \
         secondaries; on a non-fissile unit-weight fixture every spilled secondary \
         must come back as exactly one relaunch"
    );
