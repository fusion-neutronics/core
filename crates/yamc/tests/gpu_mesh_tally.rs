...
//!       --test gpu_mesh_tally -- --nocapture --test-threads=1
...
#[test]
fn gpu_cpu_mesh_flux_parity() {
    let nuclide = "C12";
    if !data_present(nuclide) || !gpu_available() {
        eprintln!("skipping gpu_cpu_mesh_flux_parity: data/GPU absent");
        return;
    }
    let density = 2.0;
    ...
    assert!(cpu_total > 0.0, "no CPU flux scored into the mesh");
    assert!(gpu_total > 0.0, "no GPU flux scored into the mesh");
    let total_ratio = gpu_total / cpu_total;
    eprintln!("mesh total flux: CPU={cpu_total:.5e} GPU={gpu_total:.5e} (ratio {total_ratio:.4})");
    assert!(
        (0.98..=1.02).contains(&total_ratio),
        "GPU/CPU total mesh flux ratio {total_ratio:.4} outside [0.98, 1.02]"
    );
    ...
    eprintln!(
        "mesh per-voxel: {n_checked} well-populated voxels, worst mean dev {:.1}%, \
         median GPU/CPU std_dev ratio {median_sd:.3}",
        100.0 * worst_mean_dev
    );
    // Independent MC estimates of the same per-voxel flux; a few % apart.
    assert!(
        worst_mean_dev < 0.06,
        "worst well-populated-voxel mean deviation {:.1}% exceeds 6%",
        100.0 * worst_mean_dev
    );
    assert!(
        (0.85..=1.18).contains(&median_sd),
        "median GPU/CPU std_dev ratio {median_sd:.3} not near 1 (per-source mesh variance biased)"
    );
}
